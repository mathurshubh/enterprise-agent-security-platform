...
Retrieves security findings generated by the Detection Engine matching optional query filters (`session_id`, `agent_id`, `severity`, `category`, `status`, `rule_id`).
    ...
    [
      {
        ...
        "status": "OPEN",
        "description": "Sensitive file access attempt detected: .env",
...
*   **Endpoint:** `GET /api/v1/findings/{finding_id}`
*   **Idempotency:** Idempotent

### List Risk Assessments

Retrieves dynamic risk assessments generated by `RiskService`. Exposes process-local derived posture.

*   **Endpoint:** `GET /api/v1/risk-assessments`
*   **Query Parameters:** `session_id`, `agent_id`, `risk_level`
*   **Idempotency:** Idempotent
*   **Response Payload:**
    ```json
    [
      {
        ...
        "risk_level": "HIGH",
        "finding_count": 2,
        "assessed_at": "2026-08-21T18:00:00Z"
      }
    ]
    ```

### Get Risk Assessment by Session ID

Retrieves the dynamic risk assessment for a specific session.

*   **Endpoint:** `GET /api/v1/risk-assessments/{session_id}`
*   **Query Parameters:** `agent_id` (optional, required if multiple risk assessments exist for the session)
*   **Idempotency:** Idempotent
*   **Responses:**
    *   `200 OK`: Returns `RiskAssessmentResponse` when exact match or single session assessment exists.
    *   `400 Bad Request`: Returned when `agent_id` is omitted and multiple risk assessments exist for the session across agents.
    *   `404 Not Found`: Returned when no assessment matches the requested session and agent.
